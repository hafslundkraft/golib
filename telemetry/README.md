...
* **WithEndpoint**: Overrides the OTLP endpoint that telemetry data is exported to. By default the endpoint is read from the `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable. The value is a host with an optional port, without scheme or path, e.g. `otel.example.com:4318`. Each signal is exported to its own default path on that host (`/v1/traces`, `/v1/metrics` and `/v1/logs`). Exports use TLS unless **WithInsecure** is set. This has no effect in local mode.
* **WithInsecure**: Exports telemetry over HTTP instead of HTTPS, e.g. to a collector reached over the cluster network. This has no effect in local mode.

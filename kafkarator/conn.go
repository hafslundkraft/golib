...
func New2(config Config, tel *telemetry.Provider) (Connection2, error) {
	d, err := dialer(config)
	if err != nil {
		return nil, err
	}
...
func (c *conn) Writer(ctx context.Context, topic string) (chan<- MessageAndContext, error) {
	...
	go func() {
		defer func() {
			if err := w.Close(); err != nil {
				c.logger.Error("failed to close writer", err)
			}
		}()
		...
		case msg := <-incomingMessages:
			...
			if err := w.WriteMessages(ctx, kafkaMessage(msg.Message.Value, headers)); err != nil {
				c.logger.Error("while writing messages to Kafka", err)
			}
			producedMessagesCounter.Add(ctx, 1)
		case <-ctx.Done():
			c.logger.Info("writer exiting")
			return
		}
...
func (c *conn) Reader(ctx context.Context, topic, consumerGroup string) (<-chan Message, error) {
	...
	go func() {
		...
		defer func() {
			if err := reader.Close(); err != nil {
				c.logger.Error("failed to close reader", err)
			}
		}()
...
func (c *conn) topicPartitions(ctx context.Context, topic string) (int, error) {
...
func (c *conn) injectTraceContext(ctx context.Context, headers map[string][]byte) map[string][]byte {
